...
from baseapp_for_restapi_backend_with_swagger import readFromEnviroment, getMissingVarFileException, VaultValueFoundWhenUsingIncompatibleReadFunction

class test_gp_getReadfromEnvironmentFn(testHelperSuperClass):
  ...
  def test_vaultVarWrongModeError(self):
      env = {
          ...
          'APIAPP_VAULT_URL': 'x',
          'APIAPP_VAULT_ROLE_ID': 'x',
          'APIAPP_VAULT_SECRET_ID': 'z'
      }
      with self.assertRaises(VaultValueFoundWhenUsingIncompatibleReadFunction) as context:
          _ = readFromEnviroment(env, "APIAPP_TEST", None, None, False)
